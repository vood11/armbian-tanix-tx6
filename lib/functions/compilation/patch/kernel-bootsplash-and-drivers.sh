compilation_prepare() {
	...
	if linux-version compare "${version}" ge 5.0 && linux-version compare "${version}" lt 5.19.2 && [[ "$LINUXFAMILY" == sunxi* ]] && [[ "$EXTRAWIFI" == yes ]]; then

		display_alert "Adding" "Wireless drivers for Xradio XR819 chipsets" "info"

		fetch_from_repo "$GITHUB_SOURCE/leojrfs/xradio" "xradio" "branch:master" "yes"
		cd "$kerneldir" || exit
		...
		if linux-version compare "${version}" ge 5.19; then
			process_patch_file "${SRC}/patch/misc/xradio01.patch" "applying"
		fi
	...
	if linux-version compare "${version}" ge 5.0 && linux-version compare "${version}" lt 6.0 && [ "$EXTRAWIFI" == yes ]; then
